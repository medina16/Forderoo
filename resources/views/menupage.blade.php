    ...
    @elseif($title == 'Search Results')
        ...
        @if (count($results) == 0)
            <p style="
                margin: 50% 30px;
                text-align: center;
                ">Hasil pencarian tidak ditemukan.</p>
        @else
            <div class="category-menus">
    ...
    @if (session()->has('tablenumber'))
        <a href="/cart">
            <button class="btn btn-success floatbar">
                <div class="checkout">
                    <p
                        style="color: var(--primary-light-active, #DDE6C4); font-size: 15px; font-style: normal; font-weight: 400; line-height: normal; letter-spacing: -0.3px; margin: auto; width: 215px;">
                        <b style="text-align: start; font-weight: 600;">Total <span
                                id="cart-total-price">@currency($totalPrice)</span></b> untuk <span
                            id="cart-total-quantity">{{ $totalQuantity }}</span> item
                    </p>
